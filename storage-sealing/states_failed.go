...
func (m *Sealing) failedCooldown(ctx statemachine.Context, sector types.SectorInfo) error {
	// TODO: Exponential backoff when we see consecutive failures

	tLog, err := m.logService.LatestLog(uint64(sector.SectorNumber))
	if err != nil {
		return err
	}
	retryStart := time.Unix(int64(tLog.Timestamp), 0).Add(minRetryTime)
...
func (m *Sealing) checkPreCommitted(ctx statemachine.Context, sector types.SectorInfo) (*miner.SectorPreCommitOnChainInfo, bool) {
...
func (m *Sealing) handleSealPrecommit1Failed(ctx statemachine.Context, sector types.SectorInfo) error {
...
func (m *Sealing) handleSealPrecommit2Failed(ctx statemachine.Context, sector types.SectorInfo) error {
	if err := m.failedCooldown(ctx, sector); err != nil {
		return err
	}
...
func (m *Sealing) handlePreCommitFailed(ctx statemachine.Context, sector types.SectorInfo) error {
	...
	if err := checkPrecommit(ctx.Context(), m.Address(), sector, tok, height, m.api); err != nil {
		...
		case *ErrInvalidDeals:
			...
			return ctx.Send(SectorInvalidDealIDs{Return: types.RetPreCommitFailed})
...
func (m *Sealing) handleComputeProofFailed(ctx statemachine.Context, sector types.SectorInfo) error {
...
func (m *Sealing) handleCommitFailed(ctx statemachine.Context, sector types.SectorInfo) error {
	...
	if err := m.checkCommit(ctx.Context(), sector, sector.Proof, tok); err != nil {
		...
		case *ErrInvalidDeals:
			log.Warnf("invalid deals in sector %d: %v", sector.SectorNumber, err)
			return ctx.Send(SectorInvalidDealIDs{Return: types.RetCommitFailed})
		case *ErrExpiredDeals:
			return ctx.Send(SectorDealsExpired{xerrors.Errorf("sector deals expired: %w", err)})
...
func (m *Sealing) handleFinalizeFailed(ctx statemachine.Context, sector types.SectorInfo) error {
	// TODO: Check sector files
...
func (m *Sealing) handleRemoveFailed(ctx statemachine.Context, sector types.SectorInfo) error {
	if err := m.failedCooldown(ctx, sector); err != nil {
		return err
	}
...
func (m *Sealing) handleTerminateFailed(ctx statemachine.Context, sector types.SectorInfo) error {
...
func (m *Sealing) handleDealsExpired(ctx statemachine.Context, sector types.SectorInfo) error {
...
func (m *Sealing) handleDealsExpiredSnapDeals(ctx statemachine.Context, sector types.SectorInfo) error {
	if !sector.CCUpdate {
...
func (m *Sealing) handleAbortUpgrade(ctx statemachine.Context, sector types.SectorInfo) error {
	if !sector.CCUpdate {
...
func (m *Sealing) handleRecoverDealIDsOrFailWith(ctx statemachine.Context, sector types.SectorInfo, failWith interface{}) error {
...
func (m *Sealing) handleSnapDealsRecoverDealIDs(ctx statemachine.Context, sector types.SectorInfo) error {
	return m.handleRecoverDealIDsOrFailWith(ctx, sector, SectorAbortUpgrade{xerrors.New("failed recovering deal ids")})
}

func recoveryPiecesToFix(ctx context.Context, api SealingAPI, sector types.SectorInfo, maddr address.Address) ([]int, int, error) {
