...
func checkTicketExpired(sector types.SectorInfo, epoch abi.ChainEpoch) bool {
	return epoch-sector.TicketEpoch > types.MaxTicketAge // TODO: allow configuring expected seal durations
}

func (m *Sealing) getTicket(ctx statemachine.Context, sector types.SectorInfo) (abi.SealRandomness, abi.ChainEpoch, error) {
...
func (m *Sealing) handlePreCommitting(ctx statemachine.Context, sector types.SectorInfo) error {
	...
	if err := checkPrecommit(ctx.Context(), m.Address(), sector, tok, height, m.api); err != nil {
		...
		case *ErrInvalidDeals:
			log.Warnf("invalid deals in sector %d: %v", sector.SectorNumber, err)
			return ctx.Send(SectorInvalidDealIDs{Return: types.RetPreCommitting})
