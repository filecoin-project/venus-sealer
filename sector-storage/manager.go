...
func New(ctx context.Context, lstor *stores.Local, stor *stores.Remote, ls stores.LocalStorage, si stores.SectorIndex, sc SealerConfig, wss WorkerStateStore, mss ManagerStateStore) (*Manager, error) {
	...
	if sc.AllowPreCommit2 {
		localTasks = append(localTasks, types.TTPreCommit2)
	}
	if sc.AllowCommit {
		localTasks = append(localTasks, types.TTCommit2)
	}
	if sc.AllowUnseal {
...
func (m *Manager) SectorsUnsealPiece(ctx context.Context, sector storage.SectorRef, offset storiface.UnpaddedByteIndex, size abi.UnpaddedPieceSize, ticket abi.SealRandomness, unsealed *cid.Cid) error {
	...
	err = m.sched.Schedule(ctx, sector, types.TTUnseal, selector, sealFetch, func(ctx context.Context, w Worker) error {
...
func (m *Manager) AddPiece(ctx context.Context, sector storage.SectorRef, existingPieces []abi.UnpaddedPieceSize, sz abi.UnpaddedPieceSize, r io.Reader) (abi.PieceInfo, error) {
	...
	err = m.sched.Schedule(ctx, sector, types.TTAddPiece, selector, schedNop, func(ctx context.Context, w Worker) error {
...
func (m *Manager) SealPreCommit1(ctx context.Context, sector storage.SectorRef, ticket abi.SealRandomness, pieces []abi.PieceInfo) (out storage.PreCommit1Out, err error) {
	...
	wk, wait, cancel, err := m.getWork(ctx, types.TTPreCommit1, sector, ticket, pieces)
	if err != nil {
...
func (m *Manager) SealPreCommit2(ctx context.Context, sector storage.SectorRef, phase1Out storage.PreCommit1Out) (out storage.SectorCids, err error) {
	...
	wk, wait, cancel, err := m.getWork(ctx, types.TTPreCommit2, sector, phase1Out)
	if err != nil {
...
func (m *Manager) SealCommit1(ctx context.Context, sector storage.SectorRef, ticket abi.SealRandomness, seed abi.InteractiveSealRandomness, pieces []abi.PieceInfo, cids storage.SectorCids) (out storage.Commit1Out, err error) {
	...
	wk, wait, cancel, err := m.getWork(ctx, types.TTCommit1, sector, ticket, seed, pieces, cids)
	if err != nil {
...
func (m *Manager) SealCommit2(ctx context.Context, sector storage.SectorRef, phase1Out storage.Commit1Out) (out storage.Proof, err error) {
	wk, wait, cancel, err := m.getWork(ctx, types.TTCommit2, sector, phase1Out)
	if err != nil {
	...
	err = m.sched.Schedule(ctx, sector, types.TTCommit2, selector, schedNop, func(ctx context.Context, w Worker) error {
...
func (m *Manager) FinalizeSector(ctx context.Context, sector storage.SectorRef, keepUnsealed []storage.Range) error {
	...
	err := m.sched.Schedule(ctx, sector, types.TTFinalize, selector,
	...
	fetchSel := newMoveSelector(m.index, sector.ID, sector.ProofType, storiface.FTCache|storiface.FTSealed, storiface.PathStorage, false)
...
func (m *Manager) ReplicaUpdate(ctx context.Context, sector storage.SectorRef, pieces []abi.PieceInfo) (out storage.ReplicaUpdateOut, err error) {
	...
	err = m.sched.Schedule(ctx, sector, types.TTReplicaUpdate, selector, m.schedFetch(sector, storiface.FTUnsealed|storiface.FTSealed|storiface.FTCache, storiface.PathSealing, storiface.AcquireCopy), func(ctx context.Context, w Worker) error {
...
func (m *Manager) ProveReplicaUpdate1(ctx context.Context, sector storage.SectorRef, sectorKey, newSealed, newUnsealed cid.Cid) (out storage.ReplicaVanillaProofs, err error) {
	...
	wk, wait, cancel, err := m.getWork(ctx, types.TTProveReplicaUpdate1, sector, sectorKey, newSealed, newUnsealed)
	if err != nil {
		return nil, xerrors.Errorf("getWork: %w", err)
	}
	defer cancel()
	...
	err = m.sched.Schedule(ctx, sector, types.TTProveReplicaUpdate1, selector, m.schedFetch(sector, storiface.FTSealed|storiface.FTCache|storiface.FTUpdate|storiface.FTUpdateCache, storiface.PathSealing, storiface.AcquireCopy), func(ctx context.Context, w Worker) error {
...
func (m *Manager) ProveReplicaUpdate2(ctx context.Context, sector storage.SectorRef, sectorKey, newSealed, newUnsealed cid.Cid, vanillaProofs storage.ReplicaVanillaProofs) (out storage.ReplicaUpdateProof, err error) {
	...
	wk, wait, cancel, err := m.getWork(ctx, types.TTProveReplicaUpdate2, sector, sectorKey, newSealed, newUnsealed, vanillaProofs)
	if err != nil {
		return nil, xerrors.Errorf("getWork: %w", err)
	}
	defer cancel()
	...
	err = m.sched.Schedule(ctx, sector, types.TTProveReplicaUpdate2, selector, schedNop, func(ctx context.Context, w Worker) error {
...
func (m *Manager) ReturnReplicaUpdate(ctx context.Context, callID types.CallID, out storage.ReplicaUpdateOut, err *storiface.CallError) error {
	return m.returnResult(ctx, callID, out, err)
}

func (m *Manager) ReturnProveReplicaUpdate1(ctx context.Context, callID types.CallID, out storage.ReplicaVanillaProofs, err *storiface.CallError) error {
	return m.returnResult(ctx, callID, out, err)
}

func (m *Manager) ReturnProveReplicaUpdate2(ctx context.Context, callID types.CallID, proof storage.ReplicaUpdateProof, err *storiface.CallError) error {
	return m.returnResult(ctx, callID, proof, err)
}
