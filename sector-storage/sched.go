...
type WorkerSelector interface {
	Ok(ctx context.Context, task types.TaskType, spt abi.RegisteredSealProof, sector storage.SectorRef, a *workerHandle) (bool, error) // true if worker is acceptable for performing a task

	Cmp(ctx context.Context, task types.TaskType, a, b *workerHandle) (bool, error) // true if a is preferred over b
}
...
func newScheduler() *scheduler {
	return &scheduler{
		...
		windowPoStSched:  newPoStScheduler(types.TTGenerateWindowPoSt),
		winningPoStSched: newPoStScheduler(types.TTGenerateWinningPoSt),
...
func (sh *scheduler) Schedule(ctx context.Context, sector storage.SectorRef, taskType types.TaskType, sel WorkerSelector, prepare WorkerAction, work WorkerAction) error {
	...
	for i := 0; i < queueLen; i++ {
		...
		go func(sqi int) {
			...
			for wnd, windowRequest := range sh.openWindows {
				...
				ok, err := task.sel.Ok(rpcCtx, task.taskType, task.sector.ProofType, task.sector, worker)
				cancel()
				if err != nil {
