...
func (m *Manager) getWork(ctx context.Context, method types.TaskType, params ...interface{}) (wid types.WorkID, wait bool, cancel func(), err error) {
	wid, err = types.NewWorkID(method, params)
	if err != nil {
		return types.WorkID{}, false, nil, xerrors.Errorf("creating WorkID: %w", err)
	}
	...
	if err != nil {
		return types.WorkID{}, false, nil, xerrors.Errorf("failed to check if the task is already tracked: %w", err)
	}

	if !have {
		err := m.work.Begin(wid, &types.WorkState{
			ID:     wid,
			Status: types.WsStarted,
		})
		if err != nil {
			return types.WorkID{}, false, nil, xerrors.Errorf("failed to track task start: %w", err)
		}
...
func (m *Manager) startWork(ctx context.Context, w Worker, wk types.WorkID) func(callID types.CallID, err error) error {
	return func(callID types.CallID, err error) error {
		var hostname string
		...
		if err != nil {
			merr := m.work.Get(wk).Mutate(func(ws *types.WorkState) error {
				ws.Status = types.WsDone
		...
		err = m.work.Get(wk).Mutate(func(ws *types.WorkState) error {
			_, ok := m.results[wk]
			if ok {
				log.Warn("work returned before we started tracking it")
				ws.Status = types.WsDone
			} else {
				ws.Status = types.WsRunning
			}
...
func (m *Manager) waitWork(ctx context.Context, wid types.WorkID) (interface{}, error) {
	m.workLk.Lock()
...
func (m *Manager) waitSimpleCall(ctx context.Context) func(callID types.CallID, err error) (interface{}, error) {
	return func(callID types.CallID, err error) (interface{}, error) {
		if err != nil {
			return nil, err
		}
...
func (m *Manager) waitCall(ctx context.Context, callID types.CallID) (interface{}, error) {
	m.workLk.Lock()
