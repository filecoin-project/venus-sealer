...
func TestReadPieceRemoteWorkers(t *testing.T) {
	...
	ppt.addRemoteWorker(t, []types.TaskType{
		types.TTPreCommit1, types.TTPreCommit2, types.TTCommit1,
		types.TTFetch, types.TTFinalize,
	})

	// create a worker that can ONLY unseal and fetch
	ppt.addRemoteWorker(t, []types.TaskType{
		types.TTUnseal, types.TTFetch,
	})
...
func newPieceProviderTestHarness(t *testing.T, mgrConfig SealerConfig, sectorProofType abi.RegisteredSealProof) *pieceProviderTestHarness {
	...
	wsts := statestore.NewDsStateStore(namespace.Wrap(dstore, datastore.NewKey("/worker/calls")))
	smsts := statestore.NewDsStateStore(namespace.Wrap(dstore, datastore.NewKey("/stmgr/calls")))
...
func (p *pieceProviderTestHarness) addRemoteWorker(t *testing.T, tasks []types.TaskType) {
	...
	dstore := ds_sync.MutexWrap(datastore.NewMapDatastore())
	csts := statestore.NewDsStateStore(namespace.Wrap(dstore, datastore.NewKey("/stmgr/calls")))
