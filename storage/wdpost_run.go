...
func (s *WindowPoStScheduler) runPoStCycle(ctx context.Context, di dline.Info, ts *types.TipSet) ([]miner.SubmitWindowedPoStParams, error) {
	...
	go func() {
		...
		if recoveries, uidMsg, err = s.declareRecoveries(context.TODO(), declDeadline, partitions, ts.Key()); err != nil {
		...
		if faults, uidMsg, err = s.declareFaults(context.TODO(), declDeadline, partitions, ts.Key()); err != nil {
	...
	for batchIdx, batch := range partitionBatches {
		...
		batchWg.Add(1)
		go func(ctx context.Context, batchIdx int, batch []types.Partition, batchPartitionStartIdx int, params miner.SubmitWindowedPoStParams) {
			defer batchWg.Done()
			...
			for retries := 0; ; retries++ {
				...
				for partIdx, partition := range batch {
					...
					partitionWg.Add(1)
					go func(partIdx int, partition types.Partition) {
...
func (s *WindowPoStScheduler) submitPoStMessage(ctx context.Context, proof *miner.SubmitWindowedPoStParams) (string, error) {
...
func (s *WindowPoStScheduler) prepareMessage(ctx context.Context, msg *types.Message, spec *types.MessageSendSpec) error {
	...
	*msg = *gm
	...
	pa, avail, err := s.addrSel.AddressFor(ctx, s.api, s.Messager, mi, api.PoStAddr, goodFunds, minFunds)
	if err != nil {
